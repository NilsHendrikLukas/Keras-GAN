...
def c2b(train, generated, adj):
    ...
    if count_nonzero(generated) <= count_nonzero(
...
def c2bcolwise(train, generated, adj):
    ...
    for col in range(len(s)):
        ...
        if count_nonzero(col_generated) <= count_nonzero(
                col_train):  # special case: number of 1 in generated is <= train, all nonzero in train = 1
...
def select_code(data, top):
    ...
    c = [x for _, x in sorted(zip(s, a), reverse=True)][
        :top]  # c contains indices correspondent to top ICD9 codes, is sorted according to frequency (from largest to smallest)
    a = zeros(len(s))
    a[c] = 1  # to one hot vector, a vector whose indices in c are 1 and all the other are 0
    data_selected = []  # store selected data
    for i in range(len(data)):
        if dot(data[i],
               a) == 0:  # if dot product is 0, this means the data vector don't have at least one of these features
            pass
        else:
            data_selected.append(data[i])
    return sorted(c), array(
        data_selected)  # index sorted in increasing order: since it is index, should be in increasing order


def data_readf(top):
    '''Read MIMIC-III data'''
    with open('/home/xieliyan/Dropbox/GPU/Data/MIMIC-III/patient_vectors_1071.pickle',
              'rb') as f:  # Original MIMIC-III data is in GPU1
...
def dwp(r, g, te, db=0.5, C=1.0):
    ...
    for i in range(len(r[0])):
        ...
        t_g[t_g >= db] = 1
        if (unique(t_r).size == 1) or (unique(
        ...
        rv_pro.append(float(count_nonzero(t_r)) / len(
            t_r))  # dimension-wise probability, see "https://onlinecourses.science.psu.edu/stat504/node/28"
...
def splitbycol(dataType, _VALIDATION_RATIO, col, MIMIC_data):
    ...
    if ((array(trainX).shape)[0] == 0 or (array(testX).shape)[
        0] == 0):  # skip column: no data point in training or testing set
        return [], []
...
def MNIST_c(file_path, data_path, path_output, digit_pair, number_train, iter, C):
    ...
    data_train_1st, label_train_1st = loaddata(digit_pair[0], 'training',
                                               data_path)  # type(label_train_1st[0]): numpy.uint8
    ...
    for i in range(len(files_1st)):
        ...
        label_1st = copy(
            label_train_1st)  # create label, in wgan.py, we generate equal number of data as training samples.
...
def loaddata(digits, dataset,
    ...
    for i in range(len(ind)):
        images[i] = array(img[ind[i] * rows * cols: (ind[
                                                         i] + 1) * rows * cols])  # every row is an image. every row: array([784 data, , , ,...])
        labels[i] = lbl[ind[i]]
